...
que viene prediseñado dentro de un paquete o en la base de R la orden es--> data(nombre del database).
...
justifiquen su respuesta al final de esta pregunta?
   A. head(co2)
B. View(co2)
C. print(co2) esta es la mejor opcion en cuanto muestra el total de datos organizadamente
D. str(co2)
...
justifiquen su respuesta al final de esta pregunta?
   A. str(co2) da a entender la cantidad de datos y la informacion contenida en las columnas
B. class(co2)
C. length(co2)
D. squeeze(co2)
...
5. Como ya lo hemos visto, usando la función mutate de
...
o lo que es lo mismo por 100,000:
...
6. La función filter de dplyr se utiliza para elegir filas
